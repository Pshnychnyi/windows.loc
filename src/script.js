...
const forms = () => {
  ...
  phoneInputs.forEach(item => {
    item.addEventListener('input', () => {
      item.value = `+${item.value.replace(/\D/g, '').slice(0, 11)}`;
    });
  });
  const message = {
    loading: 'Загрузка...',
...
const modals = () => {
  ...
  bindModal('.contact_us_wrap .phone_link', '.popup', '.popup_dialog .popup_close');
...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(document).ready(function () {
  const glazingSlider = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.glazing_slider');
  const decorationSlider = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.decoration_slider');
